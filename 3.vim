...
for step in one
 ...
 while i < n
   let pos[axis] += val
   let pos['steps'] += 1
   let visited[pos['x'] . ',' . pos['y']] = pos['steps']
   let i += 1
 endwhile
endfor
...
for step in two
 ...
 while i < n
   let pos[axis] += val
   let pos['steps'] += 1
   let nSteps = get(visited, pos['x'] . ',' . pos['y'])
   if nSteps
     call add(dists, nSteps + pos['steps'])
   endif
   let i += 1
 endwhile
endfor
